...
- Failure: Bundle generation fails
  ...
  - User-visible outcome (if any): Error logged, no UI feedback
  - Evidence: macapp/MeetingListenerApp/Sources/AppState.swift :: catch { StructuredLogger.shared.error("Debug export failed", error: error) }

- Failure: Zip process fails
  - Where detected: Process.waitUntilExit()
  - Current handling (as evidenced): Throws, no explicit check
  - User-visible outcome (if any): Export fails silently
  - Evidence: macapp/MeetingListenerApp/Sources/SessionBundle.swift :: process.waitUntilExit()
...
- Failure: File copy fails (permissions, disk full)
  ...
  - User-visible outcome (if any): Error logged
  - Evidence: macapp/MeetingListenerApp/Sources/SessionBundle.swift :: try fm.copyItem(at: zipURL, to: destinationURL)
...
- Test gaps to add (note only, no implementation): Export with audio enabled, export cancellation, disk full scenarios</content>
  <parameter name="filePath">/Users/pranay/Projects/EchoPanel/docs/flows/STO-007.md
