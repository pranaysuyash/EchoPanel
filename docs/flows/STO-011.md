...
1. For each session directory, reads metadata.json to extract started_at timestamp
   - Evidence:
...
1. Checks if transcript.jsonl file exists to determine hasTranscript flag
   - Evidence:
...
1. Builds tuple (id, date, hasTranscript) for each valid session
   - Evidence:
...
1. Returns sessions sorted by date descending (newest first)
   - Evidence:
