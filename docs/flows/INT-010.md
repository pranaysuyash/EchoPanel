...
2. Apply 10-minute sliding window filter to transcript
   - Evidence:
...
3. _filter_window calculates window start from max timestamp minus 600 seconds
   - Evidence:
...
4. Filter segments that overlap with the window
   - Evidence:
...
5. Run full analysis on filtered window (entities or cards)
   - Evidence:
...
6. Send analysis updates to client
   - Evidence:
