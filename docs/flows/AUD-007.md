...
2. Prewarm pyannote pipeline at server startup (optional, enabled by default)
   - Evidence:
     - Code: server/main.py :: prewarm_diarization_pipeline(timeout_seconds=...)
     - Code: server/services/diarization.py :: async def prewarm_diarization_pipeline(timeout_seconds: float = 120.0) -> bool

3. Load pyannote pipeline if not cached
   - Evidence:
...
4. Concatenate PCM buffers from all sources
   - Evidence:
...
5. Convert PCM16 to float32 waveform tensor
   - Evidence:
...
11. Include diarization results in final summary
    - Evidence:
...
- Pipeline loading failure:
  ...
  - Current handling: Log error, return None (prewarm task logs and continues startup)
  - User-visible outcome: No speaker labels
