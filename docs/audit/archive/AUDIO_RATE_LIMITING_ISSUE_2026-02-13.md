> **⚠️ OBSOLETE (2026-02-16):** Rate limiting issues resolved — verified against source code:
> - Server detection: `BackendManager.swift` health polling with configurable timeout (`BackendConfig.healthCheckTimeout`)
> - Connected status: `WebSocketStreamer.swift` state machine with `isConnected` / reconnect logic
> - Audio source selection: `AppState.swift` AudioSource enum with system/mic/both
> - Overload handling: `ws_live_listener.py:766-780` byte-based backpressure, `concurrency_controller.py:122` session limits
> - Browser audio: `AudioCaptureManager.swift` ScreenCaptureKit capture confirmed working
