> **⚠️ OBSOLETE (2026-02-16):** Model matrix findings resolved — verified against source code:
> - Default model `base.en`: `.env` line 1, `server/services/asr_stream.py:27` default config
> - HF token propagation: `server/main.py:21-33` `_sync_huggingface_token_env()` sets `HF_TOKEN` + `HUGGINGFACE_HUB_TOKEN`
> - Runtime health: `curl /health` → `{"model_ready":true,"model_state":"READY","warmup_time_ms":679}`
