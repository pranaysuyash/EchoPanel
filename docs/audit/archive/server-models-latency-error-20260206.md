> **⚠️ OBSOLETE (2026-02-16):** Core findings from this Feb 6 audit are resolved:
> - Model config: `.env` uses `base.en`, `/health` returns READY, `/model-status` shows warmup 679ms
> - Multiple providers: 4 registered (faster_whisper, whisper_cpp, mlx_whisper, onnx)
> - Metrics: 1Hz WS metrics with `avg_infer_ms`, `realtime_factor`, `provider_health`
> - Diarization: Session-end per-source via pyannote.audio with prewarm task
> Code evidence: `server/main.py` lifespan, `server/services/model_preloader.py`, runtime `/health` verified.
> Moved to archive.
