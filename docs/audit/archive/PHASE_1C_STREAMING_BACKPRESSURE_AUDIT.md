> **⚠️ OBSOLETE (2026-02-16):** Core backpressure and streaming reliability findings resolved:
> - Client send queue: `WebSocketStreamer.swift` bounded `OperationQueue` with 5s timeout
> - Server queue: 2s bounded (`QUEUE_MAX_SECONDS=2.0` in `ws_live_listener.py:30`)
> - RTF metrics: per-source `(processing_time, audio_duration)` samples (`ws_live_listener.py:89`)
> - Concurrency controller: `concurrency_controller.py:59` with session limits + backpressure levels
> - Degrade ladder: `degrade_ladder.py` with adaptive chunk/model/VAD adjustments
> - Recording lane (lossless): `RECORDING_LANE_ENABLED` in `ws_live_listener.py:50-54`
> Moved to archive.
