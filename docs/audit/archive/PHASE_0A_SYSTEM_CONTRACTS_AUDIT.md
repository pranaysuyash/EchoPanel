> **⚠️ OBSOLETE (2026-02-16):** Core contract findings resolved — verified against source code:
> - Correlation IDs: `ws_live_listener.py:1196-1197` — `state.attempt_id`, `state.connection_id` set from start message
> - Client validation: `WebSocketStreamer.swift` — drops messages with mismatched `attempt_id`
> - Segment IDs: `server/services/transcript_ids.py` — deterministic content hash of `(source, t0, t1, text)`
> - Client segment IDs: `macapp/MeetingListenerApp/Sources/TranscriptIDs.swift` — matching Swift implementation
> - Regression test: `tests/test_ws_segment_ids.py` — verifies `segment_id` on `asr_final` events
> Minor remaining items (session_ack, streaming-before-ASR-proven) are non-blocking. Moved to archive.
