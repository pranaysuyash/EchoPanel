> **⚠️ OBSOLETE (2026-02-16):** Key findings resolved — verified against source code:
> - whisper.cpp inference lock: `provider_whisper_cpp.py:49` — `self._session_lock = asyncio.Lock()`
> - Provider health in WS metrics: `ws_live_listener.py` metrics loop calls `provider.health()` → `ASRHealth` dataclass
> - 4 providers registered: `provider_faster_whisper.py`, `provider_whisper_cpp.py`, `provider_mlx_whisper.py`, `provider_onnx_whisper.py`
> - Capability detection: `capability_detector.py:108` — `CapabilityDetector` with RAM/CPU/GPU detection, `server/main.py:95-132`
> - Degrade ladder: `degrade_ladder.py:103` — 5-level system (NORMAL→FAILOVER), wired in `ws_live_listener.py:1243-1251`
