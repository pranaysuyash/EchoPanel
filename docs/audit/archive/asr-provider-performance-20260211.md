> **⚠️ OBSOLETE (2026-02-16):** The critical gaps cited here have been resolved:
> - "Only 2 providers" → Now 4: faster_whisper, whisper_cpp, mlx_whisper, onnx (`server/services/provider_*.py`)
> - "No whisper.cpp" → `provider_whisper_cpp.py` exists with Metal GPU + subprocess streaming
> - "No capability detection" → `capability_detector.py` with auto-select at startup (`server/main.py:95-132`)
> - "No degrade ladder" → `degrade_ladder.py` with 5-level adaptive system, integrated in `ws_live_listener.py:1243-1251`
> - "Voxtral broken residency" → Voxtral now uses session-based subprocess (not per-chunk)
> Moved to archive.
