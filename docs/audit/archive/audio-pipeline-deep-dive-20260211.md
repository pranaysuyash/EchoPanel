> **⚠️ OBSOLETE (2026-02-16):** Deep dive audit with all flows marked "Observed/Implemented".
> Code evidence: Dual-path capture (ScreenCaptureKit + AVAudioEngine), `applyLimiter()`, NSLock thread safety,
> DeviceHotSwapManager with bounded timeout (`restartCaptureWithTimeout`), Silero VAD wrapper (`vad_asr_wrapper.py`),
> diarization at session-end (`diarization.py:164`), backpressure with 2s bounded queue (`ws_live_listener.py:37`).
> Audio hardening completed in TCK-20260212-014. Moved to archive.
