> **⚠️ OBSOLETE (2026-02-16):** Streaming ASR + NLP audit findings addressed:
> - Analysis loop: 12s entities, 28s cards with `asyncio.wait_for` timeouts
> - Incremental processing: `extract_entities_incremental()`, `extract_cards_incremental()` in `analysis_stream.py`
> - Rolling summary: `generate_rolling_summary()` with 10-min sliding window
> - Card deduplication: fuzzy text matching `_fuzzy_match()` with 0.7 threshold
> Keyword-based NLP is a known v0.2 limitation (LLM deferred to v0.3). Moved to archive.
...
**Note on `state.tasks`:** This field is not currently populated but is an intentional hook for future general-purpose tasks (audio dump, metrics, etc.). The cleanup path correctly includes it in the finally block. Do not remove.
...
| CL-1 | state.tasks unpopulated | Intentional future hook | Not a bug; keep for extensibility | N/A |
...
| (Removed — `state.tasks` is intentional extensibility hook) | N/A | N/A |
