...
> - FM-2 (cold start): `server/services/model_preloader.py` ModelManager eager load, `server/main.py:148-154` lifespan warmup
> - FM-3 (queue metrics): `ws_live_listener.py:702-809` — per-source `queue_depth`, `dropped_total`, `realtime_factor`
> - FM-4 (analysis pile-up): `ws_live_listener.py:955-958` — `asyncio.wait_for` timeouts on NLP calls
> - FM-6 (reconnect cap): `ResilientWebSocket.swift` — max 5 attempts, `.error` status after cap
> - FM-10 (binary source tag): `ws_live_listener.py:1083-1112` — EP header with source byte (0=system, 1=mic)
> - FM-11 (pong timeout): `ResilientWebSocket.swift:227` — 15s dead connection detection
> - Dual pipeline: `ws_live_listener.py:50-61` — `RECORDING_LANE_ENABLED` lossless recording alongside realtime
