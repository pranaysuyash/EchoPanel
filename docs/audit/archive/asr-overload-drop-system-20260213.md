> **⚠️ OBSOLETE (2026-02-16):** ASR overload/drop findings resolved — verified against source code:
> - put_audio fix: `ws_live_listener.py:766-780` — byte-based backpressure, no ConcurrencyController enqueue
> - Regression test: `tests/test_put_audio_does_not_enqueue_controller.py` — verifies put_audio isolation
> - Auto provider selection: `server/services/capability_detector.py:108` — `CapabilityDetector.recommend()`
> - Degrade ladder: `server/services/degrade_ladder.py:103` — 5-level adaptive system integrated in `ws_live_listener.py:1243-1251`
