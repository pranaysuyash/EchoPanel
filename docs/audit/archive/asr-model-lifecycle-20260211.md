> **⚠️ OBSOLETE (2026-02-16):** All flows (MOD-001 through MOD-010) documented here are implemented.
> Code evidence: `ModelManager` with load→warmup→ready states (`server/services/model_preloader.py`),
> `shutdown_model_manager()` in lifespan (`server/main.py:188-194`), `CapabilityDetector` (`server/services/capability_detector.py`),
> `DegradeLadder` (`server/services/degrade_ladder.py`), 3 provider implementations registered.
> Moved to archive.
