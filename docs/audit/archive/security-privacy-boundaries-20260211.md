> **⚠️ OBSOLETE (2026-02-16):** Core security findings addressed:
> - Token in headers: `BackendConfig.swift:44-49` sends `Authorization: Bearer` + `x-echopanel-token`
> - Keychain storage: `KeychainHelper.swift` with `kSecAttrAccessibleAfterFirstUnlock`
> - TLS for remote: `BackendConfig.swift` uses wss:// for non-localhost
> - HTTP auth enforcement: `_require_http_auth()` in `server/main.py:51-57`
> - Debug dump bounded: Age/size/count limits in `ws_live_listener.py:39-44`
> - StructuredLogger PII redaction: Regex patterns for API tokens, file paths
> Note: Server still accepts `?token=` for backward compatibility — documented design choice, not a bug.
> Moved to archive.
