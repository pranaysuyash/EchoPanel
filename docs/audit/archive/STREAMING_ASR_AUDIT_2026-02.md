> **⚠️ OBSOLETE (2026-02-16):** Core streaming ASR findings resolved:
> - Model preload: eager warmup via `model_preloader.py`, `/health` returns READY
> - Provider abstraction: 4 providers registered, auto-select via `capability_detector.py`
> - Concurrency: per-source ASR tasks, inference locks, concurrency controller
> - Auth: headers not query string (`BackendConfig.swift:44-49`)
> - Backpressure: 2s bounded queue, degrade ladder, recording lane
> Code evidence: `server/main.py` lifespan, `ws_live_listener.py`, runtime health verified.
> Moved to archive.
